...
class AccountForm(FlaskForm):
    creator = StringField('Name', validators=[InputRequired(), Length(min=5, max=20), NoneOf([' '], message='No spaces allowed')])
    email = StringField('E-Mail', validators=[InputRequired(), Email()])
    password = PasswordField('Password', validators=[InputRequired(), Length(min=5, max=20), EqualTo('password2', message='Passwords must match')])
...
class DelCreatorForm(FlaskForm):
    confirmation = StringField('Confirmation (enter delete)', validators=[InputRequired(), Regexp("delete", message='Enter delete to confirm')])
